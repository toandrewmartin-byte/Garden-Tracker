...
`index.html`, `manifest.webmanifest`, `sw.js`, `icon.svg`, `icon-192.png`, `icon-512.png`, `apple-touch-icon.png`
...
3. Copy the `firebaseConfig` values it shows. In `index.html`, find `FIREBASE_CONFIG` near the top of the script and replace each `PASTE_...` value with yours.
...
2. Click **uploading an existing file**, drag in all 7 files, and click **Commit changes**.
...
- It works offline and syncs when the phone reconnects.

## If something goes wrong

- **"The garden database said no":** the Firestore rules weren't published. Redo step 1.5.
- **"Could not load Firebase":** a `PASTE_...` value is still in `FIREBASE_CONFIG`, or the phone is offline the first time it opens.
